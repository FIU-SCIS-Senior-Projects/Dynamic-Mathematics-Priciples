...
$GLOBALS['dbConnection'] = mysqli_connect("localhost", "root", "", "lr");
...
function user_exists($username){
	...
	$res = mysqli_query($GLOBALS['dbConnection'], "SELECT COUNT(`user_id`) FROM `users` WHERE `username` = '$username'");
	$res->data_seek(0);
    $datarow = $res->fetch_array();
...
function email_exists($email){
	...
	$res = mysqli_query($GLOBALS['dbConnection'], "SELECT COUNT(`user_id`) FROM `users` WHERE `email` = '$email'");
	$res->data_seek(0);
    $datarow = $res->fetch_array();
	return ($datarow[0] == 1) ? true : false;
}
